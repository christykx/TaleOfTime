...
router.get('/',verifyLogin,(req,res)=>{
    ...
    res.render('user/otplogin',{"authErr":req.session.authErr})
...
router.post('/sendOTP', verifyLogin,(req, res) => {
    ...
    userHelpers.otpLogin(req.body.phone).then((response)=>{
		...
		client.messages
			.create({
				body: `Your One Time Login Password For TALE OF TIME is ${otp}. Will expire in 5 minutes.`,
				from: twilioNum,
				to: phone
			})
		...
		res.render('user/enterotp',{ phone, hash: fullHash });          // Use this way in Production
	}).catch((err)=>{
		req.session.authErr=err
		res.redirect('/otplogin')
	})
